...
import { consumeAccessCode } from "../../../lib/accessCode";
...
export async function POST(request: Request) {
  ...
  if (!body.accessCode || !body.schoolId || !body.programId) {
    return NextResponse.json({ error: "Missing accessCode, schoolId or programId" }, { status: 400 });
  }
  ...
  const consumeResult = await consumeAccessCode(body.accessCode);
  if (!consumeResult.ok) {
    return NextResponse.json(
      { error: consumeResult.reason ?? "Access code is invalid", remainingUses: consumeResult.remainingUses },
      { status: 403 },
    );
  }
...
- openingQuestion should start topic 1.`;
  ...
  return NextResponse.json({
    ...
    openingQuestion,
    remainingUses: consumeResult.remainingUses,
  });
}
